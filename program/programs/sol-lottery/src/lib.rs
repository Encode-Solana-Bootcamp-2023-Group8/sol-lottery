...
declare_id!("3w5QAVGCLEBgxN1wJ2C19UuykJuJNVHTtC8uvZQ8JcAc");
...
pub mod sol_lottery {
    ...
    pub fn create_lottery(ctx: Context<CreateLottery>, ticket_price: u64) -> Result<()> {
        ...
        msg!("Lottery created: {}", lottery.id);
    ...
    pub fn pick_winner(ctx: Context<PickWinner>, _lottery_id: u32) -> Result<()> {
        ...
        let pseudo_random_number = ((u64::from_le_bytes(
            <[u8;8]>::try_from(&hash(&clock.unix_timestamp.to_be_bytes()).to_bytes()[..8]).unwrap(),) * clock.slot) % u32::MAX as u64) as u32;
    ...
    pub fn claim_prize(ctx: Context<ClaimPrize>, _lottery_id: u32, _ticket_id: u32) -> Result<()> {
        ...
        match lottery.winner_id {
            ...
            None => {
                return err!(LotteryError::WinnerNotChosen);
            }
        }
        ...
        **winner.to_account_info().try_borrow_mut_lamports()? += prize;
        ...
        msg!("{} claimed {} lamports from lottery id {} with ticket id {}", winner.key(), prize, lottery.id, ticket.id);
